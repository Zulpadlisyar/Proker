  ...
  function renderActivitiesPage() {
    ...
    if (fullCalendarContainer) {
      ...
      const monthMap = {
        ...
        'AGU': { num: 8, year: 2026 },
        'SEP': { num: 9, year: 2026 },
        'OKT': { num: 10, year: 2026 },
        'NOV': { num: 11, year: 2026 },
      ...
      function getEventStartDate(ev) {
        ...
        const mInfo = monthMap[mKey] || { num: 9, year: 2026 };
        let day = 1;
        if (ev.date) {
      ...
      const now = new Date();
      const referenceDate = now.getFullYear() >= 2026 ? now : new Date(2026, 8, 1);
      ...
      function applyCalendarFilter() {
        let listWithDates = allCalendarEvents.map(e => ({
          ...e,
          _startDate: getEventStartDate(e),
          _endDate: getEventEndDate(e)
        }));

        listWithDates.sort((a, b) => a._startDate - b._startDate);

        let filtered = listWithDates;
        const isNearestMode = (currentCalFilter === 'nearest');

        if (isNearestMode) {
          const upcoming = listWithDates.filter(e => e._endDate >= referenceDate || e._startDate >= referenceDate);
          filtered = (upcoming.length > 0 ? upcoming : listWithDates).slice(0, 5);
        } else if (currentCalFilter !== 'all') {
        ...
        const statusBarHtml = isNearestMode
          ? `
            <div class="calendar-status-bar">
              ...
              <button type="button" class="calendar-toggle-all-btn" id="btn-show-all-cal">Lihat Semua Agenda (${allCalendarEvents.length}) →</button>
            </div>
          `
          : (currentCalFilter === 'all' && !currentCalSearch
            ? `
              <div class="calendar-status-bar">
                <span class="calendar-status-text">
                  <span>Menampilkan seluruh <strong>${allCalendarEvents.length} agenda akademik</strong> tahunan</span>
                </span>
        ...
        const cardsHtml = filtered.map((ev, idx) => {
          ...
          return `
            <div class="calendar-vertical-card ${idx === 0 && isNearestMode ? 'is-nearest' : ''}">
        ...
        fullCalendarContainer.innerHTML = statusBarHtml + cardsHtml;

        const btnShowAll = document.getElementById('btn-show-all-cal');
        if (btnShowAll && calendarFilterTabs) {
          btnShowAll.addEventListener('click', () => {
            const allBtn = calendarFilterTabs.querySelector('[data-cal-filter="all"]');
            if (allBtn) allBtn.click();
          });
        }

        const btnShowNearest = document.getElementById('btn-show-nearest-cal');
        if (btnShowNearest && calendarFilterTabs) {
          btnShowNearest.addEventListener('click', () => {
            const nearestBtn = calendarFilterTabs.querySelector('[data-cal-filter="nearest"]');
